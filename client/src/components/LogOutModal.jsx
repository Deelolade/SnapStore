...
const LogOutModal = ({isOpen, onClose, title, children}) => {
  const { signOut } = useClerk()
  const dispatch = useDispatch()
  const navigate = useNavigate();
  const handleLogout = () => {
    signOut() // Firebase: auth.signOut();  Custom: clear token, call backend
    onClose()
    dispatch(logOut())
    navigate("/")
  }
        ...
        <div className="fixed inset-0 z-10 w-screen overflow-y-auto bg-black/50 backdrop-blur-sm">
          <div className="flex min-h-full items-center justify-center p-4">
            ...
            >
              ...
              <Button
                className=" p-2 rounded-lg bg-gray-600 hover:bg-gray-500 text-white"
                onClick={onClose}
              >
                Cancel
              </Button>
              <Button
                className="bg-red hover:bg-red/90 p-2 rounded-lg text-white"
